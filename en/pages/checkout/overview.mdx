...
<Note>
  Hosted checkout accepts methods **enabled on the project**: **PIX**, **crypto**, and **credit card (beta)**. Pass them in `paymentMethods` (default `["pix"]`).
</Note>

<Note>
  To use credit card in hosted checkout, contact the sales team first to discuss and enable beta access: [talk on WhatsApp](https://wa.me/5521992614614) or [send an email](mailto:contato@payments.bob.company).
</Note>
