...
<Note>
  Hosted checkout accepts methods **enabled on the project**: **PIX**, **crypto (beta)**, and **credit card (beta)**. Pass them in `paymentMethods` (default `["pix"]`).
</Note>

<Note>
  To use credit card or crypto in hosted checkout, contact the sales team first to discuss and enable beta access: [talk on WhatsApp](https://wa.me/5521992614614) or [send an email](mailto:contato@payments.bob.company).
</Note>
...
- **Crypto (beta)** — pass `crypto` in `paymentMethods` only after talking to the sales team. The buyer is taken to the hosted crypto page via the same `checkoutUrl`; confirmation arrives via `transaction.paid` just like the other methods.
