...
<Note>
  **Credit card is coming soon.** Today the hosted checkout works with **PIX**; the references to `credit_card` below apply once card payments launch.
</Note>
...
<Steps>
  ...
  <Step title="Redirect the buyer">
    Send the buyer to the `checkoutUrl`. They complete the payment using the available method (PIX).
  </Step>
...
- **`paymentMethods` controls the displayed options** — today, use `["pix"]` (the default). Support for `["credit_card"]` arrives once card payments launch.
