...
A **checkout session** connects the customer to the payment, stores the charge details, and loads the available methods (**PIX**, **crypto**, and **card**, depending on what's enabled on the project). You create a session for each charge when you use the **hosted checkout** or the **Checkout SDK**.
...
| `paymentMethods` | Methods shown to the buyer: `pix` (default), `crypto`, `credit_card` (and combinations), if enabled on the project. |
