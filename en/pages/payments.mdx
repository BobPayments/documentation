...
| **Crypto** | When enabled on the project | Hosted checkout or `paymentMethod: "crypto"` on the API | Hosted page `checkoutUrl` and status events |

<Note>
  Credit card integration is in **beta**. Contact the sales team on [WhatsApp](https://wa.me/5521992614614) or by [email](mailto:contato@payments.bob.company) to discuss and enable your project. A method can only be used when it's **enabled on the project**.
</Note>
