...
|---|---|---|---|
| **PIX** | Available | Direct API, hosted checkout, or Checkout SDK | Copy-and-paste code (`pixCode`), QR Code, and status events |
| **Credit card (beta)** | Beta, when enabled on the project | Hosted checkout, Checkout SDK, or direct API with `btk_...` token | Browser tokenization, 3DS (`nextActionUrl`), and status events |
| **Crypto (beta)** | Beta, when enabled on the project | Hosted checkout or `paymentMethod: "crypto"` on the API | Hosted page `checkoutUrl` and status events |

<Note>
  Credit card and crypto integrations are in **beta**. Contact the sales team on [WhatsApp](https://wa.me/5521992614614) or by [email](mailto:contato@payments.bob.company) to discuss and enable your project. A method can only be used when it's **enabled on the project**.
</Note>
...
<Steps>
  ...
  <Step title="Create the payment">
    Use a direct transaction (`paymentMethod` omitted or `"pix"`, `"credit_card"`, or `"crypto"`) or create a checkout session with `paymentMethods` (e.g. `['pix']`, `['crypto']`, `['credit_card', 'pix']`).
  </Step>
  <Step title="Show the next step to the buyer">
    For PIX, display the `pixCode` and the QR Code. For crypto, redirect the buyer to `checkoutUrl`. For card, use hosted checkout/Checkout SDK or the direct API with a `btk_...` token (and `nextActionUrl` if 3DS is required).
  </Step>
