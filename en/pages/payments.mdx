...
| **Credit card** | When enabled on the project | Hosted checkout, Checkout SDK, or direct API with `btk_...` token | Browser tokenization, 3DS (`nextActionUrl`), and status events |
...
<Note>
  A method can only be used when it's **enabled on the project**. The API enum lists contract methods; creation fails if the method is not published for your account.
</Note>
