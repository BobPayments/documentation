...
| **Cartão de crédito (beta)** | Beta, quando habilitado no projeto | Checkout hospedado, SDK Checkout ou API direta com token `btk_...` | Tokenização no browser, 3DS (`nextActionUrl`) e eventos de status |
| **Cripto (beta)** | Beta, quando habilitado no projeto | Checkout hospedado ou `paymentMethod: "crypto"` na API | `checkoutUrl` da página hospedada e eventos de status |

<Note>
  As integrações com cartão de crédito e cripto estão em **beta**. Fale com o time de vendas pelo [WhatsApp](https://wa.me/5521992614614) ou [e-mail](mailto:contato@payments.bob.company) para avaliar e habilitar o seu projeto. Um método só pode ser usado quando estiver **habilitado no projeto**.
</Note>
