...
O payload de webhook é **versionado**. A versão é fixada por canal no dashboard:
...
<Note>
  Os exemplos abaixo mostram o formato **v1** (legado). Na **v2**, `data.amount` vira `data.amountCents`, o envelope ganha `apiVersion: "v2"` e entra `data.paymentMethod`.
</Note>
