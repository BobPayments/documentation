...
Na **v2**, `data` também pode incluir `checkoutUrl` quando a transação tiver checkout hospedado (ex.: cripto). Esses campos **não** existem no payload v1 (formato congelado).

## Eventos disponíveis

<Note>
  Os exemplos abaixo mostram o formato **v1** (legado). Na **v2**: `data.amount` vira `data.amountCents`, o envelope ganha `apiVersion: "v2"`, e entram `data.paymentMethod` e (quando houver) `data.checkoutUrl`.
</Note>
