...
<ResponseField name="paymentMethod" type="string">
  Método de pagamento da transação: `pix` (default na criação), `credit_card`, `crypto` ou `boleto`.
</ResponseField>
...
<ResponseField name="gatewayAmountCents" type="number | null">
  Valor cobrado no gateway na moeda de liquidação (ex.: centavos de USD em cripto). `null` quando liquida em BRL.
</ResponseField>

<ResponseField name="gatewayCurrency" type="string | null">
  Moeda de liquidação do gateway (ISO 4217, ex.: `USD`). `null` = BRL.
</ResponseField>

<ResponseField name="fxRate" type="number | null">
  Cotação BRL→moeda do gateway congelada na criação. `null` quando não há conversão.
</ResponseField>

<ResponseField name="checkoutUrl" type="string | null">
  URL do checkout hospedado quando houver (ex.: pagamento em cripto).
</ResponseField>
