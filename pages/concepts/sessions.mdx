...
Uma **sessão de checkout** conecta o cliente ao pagamento, guarda os detalhes da cobrança e carrega os métodos disponíveis (**PIX**, **cripto** e **cartão**, conforme habilitados no projeto). Você cria uma sessão para cada cobrança quando usa o **checkout hospedado** ou o **SDK Checkout**.
...
| `paymentMethods` | Métodos exibidos ao comprador: `pix` (default), `crypto`, `credit_card` (e combinações), se habilitados no projeto. |
