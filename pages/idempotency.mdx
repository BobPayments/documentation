---
title: "Idempotência"
description: "Repita requisições de criação com segurança — sem risco de PIX duplicado."
icon: "rotate"
---

Timeouts e falhas de rede deixam uma dúvida perigosa: *a cobrança foi criada ou não?* Repetir às cegas pode gerar **dois PIX** para o mesmo pedido. O header `Idempotency-Key` resolve isso: com ele, repetir a mesma requisição é sempre seguro.

## Como funciona

Envie um identificador único por operação no header `Idempotency-Key` do `POST /api/v1/transactions`:
...
| Repetição com a mesma chave e mesmo body | A API **replaya a resposta original** (com header `x-idempotent-replayed: true`) — nenhum PIX novo é criado |
...
- A idempotência complementa (não substitui) a deduplicação automática da plataforma, que já bloqueia PIX repetido do mesmo comprador para o mesmo valor/produto enquanto a cobrança anterior não expira.
